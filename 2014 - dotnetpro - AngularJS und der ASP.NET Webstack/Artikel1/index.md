...
Um JavaScript-Dateien nicht mehr über Script-Tags laden zu müssen, bedient man sich gemeinhin eines Modul-Loaders. In der dotnetpro 11/2014 wurde als Modul-Loader Browserify vorgestellt - welcher das "CommonJS"-Format verwendet. Das CommonJS-Format ist erkennbar an der Verwendung von `module.exports`. CommonJS-Module sind für eine synchrone Verwendung ausgelegt. Fordert man ein CommonJS: an und diese ist noch nicht bekannt, so kann erst dann der Programmablauf fortführen, wenn die Datei vollständig geladen wurde. rst jedoch nicht für eine asynchrone Verwendung im Browser ausgelegt!

Einen ganzen Strauß an technischen Problemen lässt sich vermeiden, wenn man seine Scripte im Browser **asynchron lädt**. Hierfür gibt es eine Reihe von Formaten und Frameworks. Als Defakto-Standard sollte man das "**Asynchronous** Module Definition (AMD)"-Format [1] kennen.  Die Referenzimplentierung von AMD wird durch das Framework require.js [2] gestellt. 

 Bitte evaluieren Sie vorab, welche Features Sie von einem Modul-Loader erwarten. Sowohl Browserify als auch AMD/require.js haben ihre Berechtigung. Im Zweifelsfall macht man mit AMD nichts falsch. Sollte das das eigene Projekt sowohl AMD als auch CommonJS-Module benötigen, so hilft curl.js [3] aus der Misere. 
